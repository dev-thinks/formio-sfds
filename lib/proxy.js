...
const FORMIO_SFDS_PATH = '/dist/formio-sfds.standalone.js'
...
export class Proxy {
  ...
  transform () {
    ...
    const {
      ...
      formiojsVersion,
      protocol = VERCEL_ENV ? 'https' : 'http',
      env = 'live'
    } = this.options

    let { source, options } = this.options

    this.setFormioSFDSVersion(formioSFDSVersion, protocol)
  ...
  setFormioSFDSVersion (version, protocol) {
    ...
    if (script) {
      if (version) {
        ...
        const url = protocol
          ? `${protocol}://${VERCEL_URL}${FORMIO_SFDS_PATH}`
          : `//${VERCEL_URL}${FORMIO_SFDS_PATH}`
        console.warn('formio-sfds script url:', url)
